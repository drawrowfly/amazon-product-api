...
class AmazonScraper {
    ...
    httpRequest({uri, method, qs, json, body, form}) {
        ...
        console.log(this.mainHost)
        const proxy = this.getProxy;
        const page = qs?.pageNumber ? `&pageNumber=${qs.pageNumber}` : ''
        const encodedUri = encodeURIComponent(uri + page)
        let url = encodedUri ? `${this.mainHost}/${encodedUri}` : this.mainHost;
        url = `http://api.scrape.do?token=${this.token}&url=${(url)}?CustomHeaders=true`
