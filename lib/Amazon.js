...
class AmazonScraper {
        ...
        return new Promise((resolve, reject) => {
            forEachLimit(
                ...
                async (item) => {
                    ...
                    // validate body
                    if (body.includes('To discuss automated access to Amazon')) {
                        throw new Error('Amazon blocked your request');
                    }
    ...
    grabReviews(body) {
        ...
        try {
            reviewListStat.children.forEach((item) => {
                ...
                const percentage = $(item.children[2]).text().trim();
        ...
        for (let key in scrapingResult) {
            ...
            try {
                scrapingResult[key].review = $(search[0]).text().trim();
            } catch (error) {
                continue;
            }
        }
    ...
    extractProductSpecifications($) {
        ...
        try {
            const specList = $('#productOverview_feature_div > div > table > tbody')
                .find('tr')
                .toArray()
                .map((x) => {
                    let data = x.children
                        .filter((y) => y.name === 'td')
                        .map((val) => {
                            const spans = val.children.filter((y) => y.name === 'span').map((z) => z.children[0].data);
                            return spans[0];
                        });
                    return [data[0], data[1]];
                });

            // console.log(123, specList, 321);
    ...
    extractProductInfromation($) {
        ...
        try {
            ...
            for (let item in bestSalesSection) {
                const rank = $(bestSalesSection[item].children[0]).text().trim();
                const category = $(bestSalesSection[item].children[1]).text().trim();
                ...
                if (item.type === 'tag' && item.name === 'ul') {
                    if (item.children) {
                        item.children.forEach((rankItem) => {
                            const rank = $(rankItem.children[0].children[0]).text().trim();
                            const category = $(rankItem.children[0].children[1]).text().trim();
    ...
    extractProductVariants($, body) {
        ...
        // extend product json meta with more variant fields
        try {
            const scriptSelector = $('#twister_feature_div');
            let scriptString = '';
            scriptSelector
                .find('script')
                .toArray()
            ...
            if (scriptString) {
                let jsonString = scriptString.split('var dataToReturn =')[1].split('return dataToReturn;')[0];
                jsonString = jsonString.trim().split(';')[0];

                if (jsonString) {
                    productJsonMeta = { ...productJsonMeta, ...JSON5.parse(jsonString) };
                }
            }
        } catch (e) {
            // continue regardless of error
        ...
        if (productJsonMeta) {
            if (productJsonMeta.colorToAsin && Object.keys(productJsonMeta.colorToAsin).length) {
                for (let item in productJsonMeta.colorToAsin) {
                    const images = productJsonMeta.colorImages[item] || [];
                    // ADD VARIATION TYPE etc
                    const variantAsin = productJsonMeta.colorToAsin[item].asin;

                    const variation_values = productJsonMeta?.asinToDimensionIndexMap?.[variantAsin] || [];

                    const variationDataMap = {};
                    for (let i = 0; i < variation_values.length; i++) {
                        const variation_value = variation_values[i];
                        variationDataMap[`variationTypeDisplayName_${i + 1}`] = productJsonMeta['dimensionsDisplay'][i];
                        variationDataMap[`variationType_${i + 1}`] = productJsonMeta['dimensionsDisplay'][i];
                        variationDataMap[`variationValue_${i + 1}`] = productJsonMeta['dimensionValuesData'][i][variation_value];
                    }

                    const variant = {
                        asin: variantAsin,
                        id: this.getVariantId(variantAsin),
                        photos: images.map((x) => x.hiRes),
                        name: item,
                    ...
                    try {
                        const price = $(`li[data-defaultasin="${productJsonMeta.colorToAsin[item].asin}"] div.twisterSlotDiv`).text().trim();
                        if (price) variant.price = this.geo.price_format(price);
                    } catch {
                        // continue regardless of error
                    }
                    if (productJsonMeta.colorToAsin[item].asin === this.asin) {
                        variant['specs'] = this.extractProductSpecifications($);
                    }
            ...
            parentAsin = productJsonMeta.parentAsin;
            ...
            try {
                for (let item of variantsSelector[0].children) {
                    ...
                    try {
                        const price = $(`li[id=${item.attribs['id']}] div.twisterSlotDiv`).text().trim();
                        if (price) variant.price = this.geo.price_format(price);
                        variant.images =
        ...
        return [variants, videos, parentAsin];
    }
    ...
    extractAlsoBought($) {
        ...
        for (let item of bought[0].children[0].children) {
            ...
            try {
                const review = $(item.children[0].children[1].children[1]).text().trim();
                if (review) {
            ...
            try {
                const price = $(item.children[0].children[2]).text().trim();
                if (price) {
    ...
    extractSponsoredProducts($) {
        ...
        for (let item of sponsored[0].children[0].children) {
            ...
            const review = $(`#sp_detail-none_${product.asin} > div:nth-child(6) > a > span`).text().trim();
            if (review) {
                product.reviews.total_reviews = parseInt(review.replace(/[^\d]/g, ''), 10);
            }

            const price = $(`#sp_detail-none_${product.asin} > div.a-row.a-color-price > a:nth-child(1) > span`).text().trim();
            if (price) {
                product.price.current_price = this.geo.price_format(price);
            }
    ...
    getRating($) {
        const reviews = $('#averageCustomerReviews');
        return reviews?.children('span')?.children('span')[0]?.attribs?.title?.split('out of')[0]?.trim();
    }
    ...
    grabAsinDetails(body) {
        ...
        output.categories = this.extractProductCategories($).map((x) => x.category);
        ...
        [output.variants, output.videos, output.parentAsin] = this.extractProductVariants($, body);
        output.total_videos = output.videos?.length || 0;

        output.description = $('#productDescription').text().trim();

        if (body.indexOf('bookDescription_feature_div') > -1 && !output.description) {
            output.description = $('#bookDescription_feature_div').text().trim();
            try {
        ...
        const deliveryMessage = $('#deliveryMessageMirId').text().trim();
        output.delivery_message = deliveryMessage;

        const merchantInfo = $('div[id="merchant-info"]').text().trim();
        if (merchantInfo) {
            try {
        ...
        output.title = $(`span[id="productTitle"]`).text() || $('.qa-title-text').text().trim();
        ...
        const tmpRating = this.getRating($);
        output.reviews.rating = tmpRating
            ? tmpRating
        ...
        output.specs = this.extractProductSpecifications($);
    ...
    grabProduct(body, p) {
        ...
        for (let key in scrapingResult) {
            try {
                ...
                const amazonChoice = $(`div[data-asin=${key}] span[id="${key}-amazons-choice"]`).text().trim();
                const bestSeller = $(`div[data-asin=${key}] span[id="${key}-best-seller"]`).text().trim();
            ...
            byAuthors.children.forEach((item, index) => {
                try {
                    if (item && item.name === 'span' && item.attribs.class.indexOf('author') > -1) {
                        let url = '';
                        let author = '';
                        let role = $(`#bylineInfo > span:nth-child(${index}) > span.contribution`).text().trim();
                        if (role) {
                        ...
                        if (item.children[0].name === 'a') {
                            ...
                            if (link != '#') {
                                ...
                                author = $(`#bylineInfo > span:nth-child(${index}) > a`).text().trim();
                            }
                        } else {
                            const link = $(`#bylineInfo > span:nth-child(1) > span.a-declarative > a.a-link-normal.contributorNameID`)[0].attribs
                                .href;
                            url = `${this.mainHost}${link}`;
                            author = $(`#bylineInfo > span:nth-child(1) > span.a-declarative > a.a-link-normal.contributorNameID`).text().trim();
                        }
                        if (author && url) {
                            authors.push({
                                author,
            ...
            moreBuyingOptions.children.forEach((item, index) => {
                try {
                    if (item.attribs.class && item.attribs.class === 'a-box mbc-offer-row pa_mbc_on_amazon_offer') {
                        position += 1;
                        const price = $(`#mbc > div:nth-child(${index + 1}) > div > span.a-declarative > div > div:nth-child(1)`)
                            .text()
                            .trim();
                        const seller = $(`#mbc-sold-by-${position} > span.a-size-small.mbcMerchantName`).text().trim();
    ...
    extractBookInSeries($) {
        ...
        if (bookSeriesName) {
            try {
                ...
                const series_name = $('div.shoveler-cell:nth-child(2) > a:nth-child(2)').text().trim();
                series.push({
